...
def send_register_email(email,send_type='register'):
    ...
    if send_type == 'register':
        ...
        email_body = u'请点击下边的链接激活你的账号：<a>http://127.0.0.1:8000/active/{0}<a>'.format(code)
        ...
        email_body = u'请点击下边的链接重置你的密码：<a>http://127.0.0.1:8000/reset/{0}</a>'.format(code)
        send_status =  send_mail(email_title,email_body,DEFAULT_FROM_EMAIL,[email])
        if send_status:
            print u'邮件发送成功！'

    elif send_type == 'update_email':
        email_title = u'MOOC修改验证码'
